...
function toSHA256(content) {
  ...
  return CryptoJS.SHA256(CryptoJS.lib.WordArray.create(a, i8a.length)).toString(
    CryptoJS.enc.Hex
  );
}
...
function isDate(date) {
  ...
  if (Object.prototype.toString.call(date) === "[object Date]") var d = date;
...
(function (ADL) {
  ...
  */
  ...
  ADL.XHR_request = function (
    ...
    // ***** rk send Terminated on close window *****
    if (window.xUnload) {
      window.xUnload = false;
      fetch(url, {
        method: "post",
        headers: headers,
        body: data,
  ...
  // If it's not cross-domain or we're not using IE, use the fetch API
  if (!xDomainRequest || !window.XDomainRequest) {
      var fetchOptions = {
          method: method,
          headers: headers,
          body: data,
      ...
      fetch(url, fetchOptions)
          ...
          .then(function (body) {
              if (callback) {
                  strictCallbacks ? callback(null, JSON.stringify(body), body) : callback(JSON.stringify(body), body);
              }
              else {
                console.log(body);
                return body;
          ...
          .catch(function (error) {
              ADL.XAPIWrapper.log("There was a problem communicating with the Learning Record Store. " + error.message);
              ADL.xhrRequestOnError(null, method, url, callback, callbackargs, strictCallbacks);
          });
  }
