var cmi5Controller = (function () {
  ...
  function setAgentProfile(r) {
    ...
    if (r.response.languagePreference || r.response.audioPreference) {
      ...
      var obj = r.response;
      //if (r.languagePreference || r.audioPreference) {
      //var obj = JSON.parse(r.response);
      //var obj = r;
  ...
  return {
    ...
    /*getAuthToken: function (successCallback, tokenErrorCallBack) {
      ...
      // We do not already have the auth token.  Make a call to fetchUrl to get it.
      fetch(cmi5Controller.fetchUrl, {
          method: 'POST',
          //credentials: 'include', // Uncomment this line if you need to include credentials
      })
      ...
      .catch(function (error) {
          console.log("Call to fetchUrl failed with error: " + error.message);
      });
  },
  */
